...
- 今日概览：热量主卡承担今天（摄入 / 健康消耗 / 当前差值 / 完整状态 / 消耗详情入口）。完整时写「按当前记录计算 · 截至 HH:mm」，缺项写未记/未计入。不单独保留「今天：多吃了 / 没有吃多」。没记饮食不算吃了 0，也不出差值。体重摘要条只展示「最近」，暂不展示目标或到达时间。有体重后再展开趋势图。点趋势图某一天或消耗详情进入日详情。热量卡用「Apple 健康已连接 / 未连接 Apple 健康」指示授权；点未连接才请求权限，已连接打开设置
- iPhone 视觉 token：`AppTheme` / `AppFont`（4pt 间距、字号角色、卡片/行组件）；品牌色与小组件共用 `BrandColor`
- 日期/时间强制简体中文（`AppLocale` / `zh-Hans`）
- 设置：个人资料在设置里编辑（概览暂不入口）；体重单位 kg/lb；iCloud 真实账号状态；健康授权入口；用户可「清除我的全部数据」（体重 / 饮食 / 运动 / 个人资料 / 小组件摘要；尝试删本应用写入健康的手动体重；清除后暂停自动从健康导入，直到用户再点同步）；完整隐私政策页（读 `PrivacyPolicy.md`）
...
16. **概览热量主卡用数字和完整状态，不用「多吃了 / 没有吃多」做单独一行。** 体重摘要条只展示「最近」，不展示目标或到达时间。`WeightPace` 仍用于最近趋势；若以后再展示到达时间，文案必须带「大约 / 粗算」，间隔不足 7 天或变化小于 0.05 kg 不报天数。标题用陈述不用问句。
