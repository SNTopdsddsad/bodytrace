...
底部 Tab 从左到右：**概览 / 饮食 / 体重**（`AppSection.allCases`）。没有运动 Tab：锻炼只从 Apple 健康导入，不可在本应用增删改，展示在概览热量卡运动概要和日详情。

1. **今日概览**
   - 热量主卡承担今天：摄入、健康消耗（活动 + 静息）、当前差值、完整状态、消耗详情入口。完整时写「按当前记录计算 · 截至 HH:mm」。不再单独保留「今天：多吃了 / 没有吃多」。没记饮食不出差值
   - 「最近」和「目标」合成体重摘要条，放在热量主卡下方；点最近记体重，点目标打开个人资料
   - 当天有锻炼时热量卡补一句运动概要（次数 + 总分钟），没有就不展示；有体重后再展开趋势图
   - 点趋势图某一天或消耗详情进入日详情（当天体重、饮食、运动、热量）。当天没有锻炼时只显示「没有运动记录」和「？」；点问号再说明如何在「健康」补记。不在本应用手写运动，也不跳转「健身」App
